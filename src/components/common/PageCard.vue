...
<template>
  <article
    class="page-card glass cursor-pointer overflow-hidden"
    role="link"
    tabindex="0"
    @keydown.enter="emit('navigate', pageId)"
  ...
  >
    ...
    <div class="p-4">
      ...
      <div
        ...
        style="font-size: var(--fs-xs); color: var(--c-text-secondary)"
      >
        <span v-if="lastSync" class="flex items-center gap-1">
          {{ formatDate(lastSync) }}
        </span>
        <span class="flex items-center gap-1"> {{ blockCount }} 块 </span>
        <span v-if="childCount > 0" class="flex items-center gap-1"> {{ childCount }} 子页 </span>
      </div>
    </div>
  </article>
</template>
