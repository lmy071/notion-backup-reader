...
function toggleDbImport() {
  enableDbImport.value = !enableDbImport.value
  save()
}
</script>
...
<template>
  <div class="notebook-page mx-auto">
    <header class="page-heading">
      <div>
        <span class="page-kicker">03 / PREFERENCES</span>
        <h1 style="color: var(--c-text-primary)">配置</h1>
      </div>
      <PixelCompanion class="heading-companion" />
    </header>

    <div
      class="paper-panel config-paper p-6 max-w-2xl"
      style="
    ...
    >
      ...
      <div class="mb-5">
        <label
          ...
          style="color: var(--c-text-secondary)"
          for="apiKey"
          >Notion Integration Token</label
        >
        <input
          id="apiKey"
          v-model="apiKey"
      ...
      <div class="mb-5">
        ...
        <input
          ...
          style="background-color: var(--c-bg); border-color: var(--c-border); color: var(--c-text)"
        />
        <p class="text-xs mt-1" style="color: var(--c-text-tertiary)">
          同时同步的页面数量，建议 1-3
        </p>
      </div>
